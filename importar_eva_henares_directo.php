<?php
// importar_eva_henares_directo.php - Script rápido para importar los 409 alumnos de Eva Henares
...
$csv_path = __DIR__ . '/alumnos_eva_henares.csv';
$error = '';
$success = '';
$procesados = 0;
$insertados = 0;
...
// Buscar ID de Eva Henares (Cuidado: NO es Eva Álvarez)
$stEva = $pdo->query("SELECT id, nombre, apellidos FROM usuarios WHERE (nombre LIKE '%Eva%' AND apellidos LIKE '%Henares%') OR (nombre LIKE '%Eva%' AND apellidos NOT LIKE '%lvarez%') ORDER BY (apellidos LIKE '%Henares%') DESC LIMIT 1");
$eva = $stEva->fetch(PDO::FETCH_ASSOC);
$comercial_id = $eva['id'] ?? null;

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ejecutar_importacion'])) {
    if (!file_exists($csv_path)) {
        $error = "El archivo CSV de la lista de Eva Henares no existe en la raíz.";
    } else {
        ...
        if ($handle) {
            ...
            while (($data = fgetcsv($handle, 0, ';')) !== false) {
                ...
                try {
                    ...
                    $error_msg = "Línea $num_linea (" . htmlspecialchars($nombre . ' ' . $primer_apellido) . "): " . $e->getMessage();
            ...
            $success = "¡Importación masiva de Eva Henares completada! Registros procesados: $procesados (Nuevos: $insertados, Actualizados: $actualizados) asignados a la comercial Eva Henares (ID #$comercial_id).$err_summary";
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Importación Lista Eva Henares - <?= APP_NAME ?></title>
...
<div class="app-container" style="display:flex; min-height:100vh;">
    ...
    <main class="main-content" style="flex:1; padding: 2rem;">
        ...
        <h1 style="color:#0f172a; font-weight:800;">Importar Lista de Alumnos de Eva Henares</h1>
        <p style="color:#64748b;">Se han detectado <strong>409 alumnos</strong> preparados para importar y asignar automáticamente a <strong>Eva Henares</strong> (no confundir con Eva Álvarez).</p>
        ...
        <?php if ($eva): ?>
            <div style="background:#f0f9ff; border:1px solid #bae6fd; padding:1rem; border-radius:8px; margin-bottom:1.5rem; color:#0369a1; font-weight:600;">
                👤 Comercial Asignado: <strong><?= htmlspecialchars($eva['nombre'] . ' ' . $eva['apellidos']) ?></strong> (ID #<?= $eva['id'] ?>)
            </div>
        <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <form method="POST">
                ...
                <button type="submit" class="btn" style="background:#16a34a; color:white; padding:1rem 2rem; font-size:1.1rem; border-radius:8px; font-weight:800; border:none; cursor:pointer;">
                    ⚡ Importar los 409 Alumnos de Eva Henares Ahora
                </button>
            </form>
        <?php endif; ?>
