...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo_excel'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, ['csv', 'xls', 'xlsx'])) {
            ...
            // Leer CSV (soporte básico; para XLSX real se necesitaría PhpSpreadsheet)
            $contenido = [];
            ...
            if ($ext === 'csv') {
                ...
                $error = "Para archivos XLS/XLSX, convierta primero a CSV (separado por punto y coma). O instale PhpSpreadsheet para soporte nativo.";
            }
            
            if (empty($error) && !empty($contenido)) {
                // Parsear rango
                $inicio_fila = 2; // Por defecto, A2 (saltando encabezados)
                $fin_fila = count($contenido);
                $inicio_col = 0; // A
                $fin_col = 12; // M (13 columnas, 0-indexed)
                
                if (!empty($rango)) {
                    // Parsear rango tipo A2:M57
                    if (preg_match('/^([A-Z])(\d+):([A-Z])(\d+)$/i', $rango, $matches)) {
                        $inicio_col = ord(strtoupper($matches[1])) - ord('A');
                        $inicio_fila = intval($matches[2]);
                        $fin_col = ord(strtoupper($matches[3])) - ord('A');
                ...
                for ($i = $inicio_fila - 1; $i < min($fin_fila, count($contenido)); $i++) {
                    if (!isset($contenido[$i])) continue;
                    
                    $fila = [];
                    $fila_vacia = true;
                    
                    for ($j = $inicio_col; $j <= min($fin_col, count($contenido[$i]) - 1); $j++) {
                        $col_index = $j - $inicio_col;
                        $col_name = $columnas[$col_index] ?? "col_$col_index";
                    ...
                    if (!$fila_vacia) {
                        ...
                        $fila['_fila_num'] = $num_fila;
                        $fila['_errores'] = $errores_fila;
                        $datos_preview[] = $fila;
                        
                        if (!empty($errores_fila)) {
                            $errores_validacion[] = "Fila $num_fila: " . implode(', ', $errores_fila);
                        }
                    }
                }
                ...
                if (!$simulacion && empty($errores_validacion) && $total_filas > 0) {
                        ...
                        foreach ($datos_preview as $fila) {
                            ...
                            if (!empty($fila['fecha_emision'])) {
                                ...
                                if (preg_match('#^(\d{1,2})/(\d{1,2})/(\d{4})$#', $fe, $m)) {
                                    ...
                                    $fecha_emision = $fe; // Intentar como está
                                }
                            }
                    ...
                    $success = "Simulación completada. Se han detectado <strong>$total_filas</strong> filas válidas" . 
                               (!empty($errores_validacion) ? " y <strong>" . count($errores_validacion) . "</strong> error(es)." : ".");
                }
            } elseif (empty($error)) {
