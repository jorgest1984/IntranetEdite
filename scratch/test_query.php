...
echo "Count: " . count($resultados) . "\n";
foreach ($resultados as $res) {
    try {
        $name = htmlspecialchars(($res['primer_apellido'] ?? '') . ' ' . ($res['segundo_apellido'] ?? '') . ', ' . ($res['alumno_nombre'] ?? ''));
        echo "Row: " . $res['id'] . " - " . $name . "\n";
    } catch (Throwable $t) {
        echo "ERR on row " . $res['id'] . ": " . $t->getMessage() . "\n";
    }
}
