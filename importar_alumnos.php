...
if (!has_permission([ROLE_ADMIN, ROLE_COORD, ROLE_TUTOR, ROLE_COMERCIAL, ROLE_PRACTICAS])) {
                    ...
                    if (!$simulacion) {
                        try {
                            if ($existe) {
                                ...
                                $stUp = $pdo->prepare("UPDATE alumnos SET nombre=?, primer_apellido=?, segundo_apellido=?, email=?, telefono=?, centro_trabajo=?, localidad=?, provincia=?, cp=?, colectivo=?, comercial_id=COALESCE(?, comercial_id) WHERE dni=? OR email=?");
                                $stUp->execute([$nombre, $primer_apellido, $segundo_apellido, $email_val, $telefono, $empresa_nombre, $localidad, $provincia, $cp, $colectivo, $comercial_id_asignado, $dni_val, $email_val]);
                                $actualizados++;
                            } else {
                                // Insertar nuevo
                                ...
                                $stIns->execute([$nombre, $primer_apellido, $segundo_apellido, $dni_val, $email_val, $telefono, $empresa_nombre, $localidad, $provincia, $cp, $colectivo, $comercial_id_asignado]);
                                $insertados++;
                            }
